...
struct FamilyFlow: View {
    ...
    private func startProcessing() {
        ...
        Task {
            ...
            do {
                let result = try await imageProcessingService.processFamily(
...
// MARK: - Family Upload View
struct FamilyUploadView: View {
    @Binding var selectedImages: [UIImage]
    @Binding var familyStyle: FamilyStyle
    let onContinue: () -> Void
    let onDismiss: () -> Void
        ...
        VStack(spacing: 0) {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: adaptiveSpacing(32, for: geometry)) {
                    ...
                    // Style Selection (only for single image)
                    if selectedImages.count == 1 {
                        FamilyStyleSelectionView(
                            familyStyle: $familyStyle,
                            geometry: geometry
                        )
                        .padding(.horizontal, adaptiveSpacing(20, for: geometry))
...
// MARK: - Family Style Selection View
struct FamilyStyleSelectionView: View {
    @Binding var familyStyle: FamilyStyle
    let geometry: GeometryProxy
        ...
        VStack(spacing: adaptiveSpacing(12, for: geometry)) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: adaptiveSpacing(12, for: geometry)) {
                    ForEach(FamilyStyle.allCases, id: \.self) { style in
                        FamilyStyleButton(
                            style: style,
                            isSelected: familyStyle == style,
                            geometry: geometry
                        ) {
                            withAnimation { familyStyle = style }
                        }
                    }
                }
...
// MARK: - Family Style Button
struct FamilyStyleButton: View {
